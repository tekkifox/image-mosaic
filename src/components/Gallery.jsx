...
const Gallery = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/photo-count?category=Travelling', { signal: controller.signal })
      .then((r) => r.json())
      .then((data) => {
        if (!mounted) return;
    ...
    fetch(`/api/tiles?category=Travelling&limit=${ITEMS_PER_PAGE}&offset=0`, { signal: controller.signal })
      .then((r) => r.json())
      .then((data) => {
        if (!mounted) return;
  ...
  const loadMorePhotos = () => {
    ...
    fetch(`/api/tiles?category=Travelling&limit=${ITEMS_PER_PAGE}&offset=${currentOffset}`)
      .then((r) => r.json())
      .then((data) => {
